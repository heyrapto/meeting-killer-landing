        ...
        <div className="flex gap-6 max-md:flex-col md:px-5 overflow-x-auto pb-6 hide-scrollbar custom-scrollbar snap-x snap-mandatory">
          {benefits.map((benefit, index) => (
            <motion.div
              ...
              variants={fadeUp}
              whileHover={{ y: -6, boxShadow: "0 10px 40px -10px rgba(0,0,0,0.05)" }}
              whileTap={{ scale: 0.995 }}
              transition={{ type: "spring", stiffness: 250, damping: 22 }}
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#335CFF]/5 rounded-bl-[100px] pointer-events-none"></div>
